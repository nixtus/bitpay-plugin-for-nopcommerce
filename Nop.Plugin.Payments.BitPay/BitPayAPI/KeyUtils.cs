...
namespace BitPayAPI
{
    public class KeyUtils
    {
        ...
        private static string GetPrivateKeyFullName()
        {
            var path = CommonHelper.MapPath("~/Plugins/Payments.Bitpay/Keys");
            if (!Directory.Exists(path))
                Directory.CreateDirectory(path);

            return Path.Combine(path, PRIV_KEY_FILENAME);
        }

        public static bool privateKeyExists()
        {
            return File.Exists(GetPrivateKeyFullName());
        }
